...
export default function Nominas() {
  ...
  const { nominas, loading: loadingNominas, createNomina, sendNomina, deleteNomina, refresh } =
    useNominas(orgId, { year: selectedYear, month: selectedMonth });

  useEffect(() => {
    document.title = 'Nóminas – TurnoSmart';
  }, []);

  // Map colaboradorId → nomina for this period
        ...
        {loadingColabs || loadingNominas ? (
          ...
          <div className="space-y-3">
            {filteredColabs.map((colab) => {
              ...
              if (!nomina) {
                // Employee has no nomina for this period
                return (
              ...
              return (
                <NominaCard
                  key={nomina.id}
                  nomina={{ ...nomina, colaborador_nombre: colab.nombre, colaborador_apellidos: colab.apellidos }}
                  showEmployee
                  isManager={isManager}
                  onSend={sendNomina}
