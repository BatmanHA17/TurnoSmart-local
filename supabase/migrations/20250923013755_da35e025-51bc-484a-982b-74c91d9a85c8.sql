-- Ajustar estructura de tabla y insertar niveles y categorías profesionales correctos
-- del convenio colectivo de hostelería de Las Palmas

-- Primero, hacer que agreement_id y org_id sean nullable para datos maestros
...
-- Limpiar datos existentes que puedan ser incorrectos
DELETE FROM public.professional_categories WHERE agreement_id IS NULL OR agreement_id IS NOT NULL;
DELETE FROM public.professional_levels WHERE org_id IS NULL OR org_id IS NOT NULL;
...
INSERT INTO public.professional_categories (category_name, category_type, level_id, description, agreement_id, org_id, created_by)
SELECT 
    categoria,
    'HOSTELERIA_LAS_PALMAS',
