-- Crear tabla para puestos predefinidos del convenio
...
-- Crear política RLS para que todos los usuarios autenticados puedan leer los puestos predefinidos
CREATE POLICY "org_members_view_predefined_jobs" ON public.predefined_jobs
FOR SELECT TO authenticated 
USING (true);

-- Crear política para que solo los admins puedan gestionar los puestos predefinidos
...
-- Insertar puestos predefinidos basados en las categorías profesionales existentes
...
CREATE INDEX idx_predefined_jobs_title ON public.predefined_jobs(job_title);
CREATE INDEX idx_predefined_jobs_category ON public.predefined_jobs(category_name);
CREATE INDEX idx_predefined_jobs_level ON public.predefined_jobs(level_name);