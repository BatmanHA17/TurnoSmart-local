-- Actualizar org_id en job_departments que son NULL
UPDATE public.job_departments 
SET org_id = 'ae3db3e5-00ec-43c3-a3de-a245de33729d'
...
-- Sincronizar datos existentes: crear rotas para equipos que no las tengan
DO $$
DECLARE
  dept_record RECORD;
BEGIN
  ...
  LOOP
    INSERT INTO public.teams (
      ...
      dept_record.value,
      'Rota generada automáticamente desde equipo: ' || dept_record.value,
      dept_record.org_id,
      true,
      COALESCE((SELECT MAX(order_index) + 1 FROM public.teams WHERE org_id = dept_record.org_id), 1)
    );
  END LOOP;
END $$;