-- ===============================
-- FASE 3: VISTA CONSOLIDADA "COLABORADOR FULL" PARA UI (CORREGIDA)
-- Crear vista + índices + validación (sin RLS directo en vista)
-- ===============================
...
-- 2. CREAR ÍNDICES PARA RENDIMIENTO OPTIMIZADO
...
-- 3. FUNCIÓN AUXILIAR PARA CONSULTAS SEGURAS DE LA VISTA
-- Esta función respeta RLS automáticamente
CREATE OR REPLACE FUNCTION get_colaborador_full_by_org(org_uuid uuid)
...
BEGIN
  ...
  RETURN QUERY
  SELECT 
    'helper_function_exists'::text,
    1::bigint,
    (SELECT COUNT(*) FROM pg_proc WHERE proname = 'get_colaborador_full_by_org')::bigint,
    CASE 
      WHEN EXISTS(SELECT 1 FROM pg_proc WHERE proname = 'get_colaborador_full_by_org')
      THEN 'PASS'::text 
      ELSE 'FAIL'::text 
    END,
    'Función auxiliar get_colaborador_full_by_org creada'::text;
  ...
  SELECT 
    ...
    1::bigint,
    CASE 
      WHEN (SELECT COUNT(*) FROM public.colaborador_full LIMIT 10) >= 0
      THEN 'PASS'::text 
      ELSE 'FAIL'::text 
    END,
    'Vista responde correctamente a consultas básicas'::text;
END;
...
-- 6. COMENTARIOS SOBRE RLS
COMMENT ON VIEW public.colaborador_full IS 'Vista consolidada de colaboradores que hereda automáticamente las políticas RLS de las tablas base (colaboradores, employee_health, employee_emergency_contacts, etc). Usar get_colaborador_full_by_org() para consultas seguras.';

COMMENT ON FUNCTION public.get_colaborador_full_by_org(uuid) IS 'Función auxiliar para consultar colaborador_full respetando RLS. Alternativa segura a consultas directas sobre la vista.';