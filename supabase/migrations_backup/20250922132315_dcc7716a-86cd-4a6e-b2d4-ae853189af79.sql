...
-- FASE 1: Constraints seguros
-- Crear índice único concurrente en (org_id, lower(email))
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_colaboradores_org_email_uniq
ON public.colaboradores (org_id, lower(email));

-- Añadir constraint unique reutilizando el índice
ALTER TABLE public.colaboradores
  ADD CONSTRAINT colaboradores_org_email_uniq
  UNIQUE USING INDEX idx_colaboradores_org_email_uniq;

-- Añadir CHECK constraint para validación de formato de email
ALTER TABLE public.colaboradores
