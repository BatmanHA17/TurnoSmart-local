-- ===============================
-- FASE 2: SEPARACIÓN DE DATOS SENSIBLES (CORREGIDA)
-- Crear tablas nuevas + Backfill + RLS + Triggers (sin duplicados)
-- ===============================
...
-- 3. CREAR ÍNDICES PARA RENDIMIENTO (solo si no existen)
...
-- 4. TRIGGERS PARA UPDATED_AT (verificar existencia primero)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_employee_health_updated_at') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_employee_emergency_contacts_updated_at') THEN
...
DROP POLICY IF EXISTS "eh_read_org_members" ON public.employee_health;
DROP POLICY IF EXISTS "eh_write_admins" ON public.employee_health;
...
DROP POLICY IF EXISTS "eec_read_org_members" ON public.employee_emergency_contacts;
DROP POLICY IF EXISTS "eec_write_managers" ON public.employee_emergency_contacts;
...
-- Trigger para sincronización automática (verificar si existe)
DROP TRIGGER IF EXISTS sync_colaborador_health_trigger ON public.colaboradores;
